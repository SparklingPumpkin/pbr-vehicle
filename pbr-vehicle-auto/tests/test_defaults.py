...
def test_viser_fitted_lighting_mappings_round_trip():
    for kind, values in (("sun_intensity", [-1.0, -0.15, 0.0, 0.15, 1.0]), ("brightness", [-1.0, -0.3, 0.0, 0.3, 1.0])):
        for ui_value in values:
            assert np.isclose(value_to_ui(ui_to_value(ui_value, kind), kind), ui_value)
