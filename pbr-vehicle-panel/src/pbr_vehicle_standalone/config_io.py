...
def state_from_config(payload: dict[str, Any], fallback: VehicleState) -> tuple[VehicleState, LightingState | None]:
    if payload.get("config_kind") == "pbr-vehicle-standalone-viewer" or isinstance(payload.get("vehicle"), dict):
        ...
        state = VehicleState(
            ...
            environment_map_resolution=int(raw.get("environment_map_resolution", 128)),
            environment_preview_visible=bool(raw.get("environment_preview_visible", False)),
            environment_preview_follow_vehicle=bool(raw.get("environment_preview_follow_vehicle", True)),
            environment_preview_position=list(map(float, raw.get("environment_preview_position", [0.0, 0.0, 0.0]))),
            environment_preview_offset=list(map(float, raw.get("environment_preview_offset", [0.0, -4.0, 2.0]))),
